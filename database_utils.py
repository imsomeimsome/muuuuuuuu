...
def add_artist(platform, artist_id, artist_name, artist_url, owner_id, genres=None):
    conn = get_connection()
    c = conn.cursor()
    c.execute(
        '''INSERT OR REPLACE INTO artists 
        (platform, artist_id, artist_name, artist_url, last_release_date, owner_id, genres)
        VALUES (?, ?, ?, ?, ?, ?, ?)''',
        (platform, artist_id, artist_name, artist_url, None, owner_id, ",".join(genres) if genres else None)
    )
    conn.commit()
    conn.close()
